...
def render_top_view(ply_path, img_save_dir, 
                   rend_size=(1024, 1024), fill_ratio=0.85,
                   render_mask=True, save_camera_params=True,
                   mask_save_dir: Optional[str] = None,
                   camera_params_save_dir: Optional[str] = None):
    """
    從單一 .ply 檔案渲染俯視影像
    
    Args:
        ply_path: .ply 檔案路徑
        img_save_dir: 純打光影像輸出目錄
        rend_size: 渲染影像尺寸 (高, 寬)
        fill_ratio: 模型填充比例 (0.0-1.0)，建議 0.85
        render_mask: 是否渲染語義標籤影像，預設 True
        save_camera_params: 是否儲存相機參數，預設 True
        mask_save_dir: mask 輸出目錄（render_mask=True 時必須提供）
        camera_params_save_dir: 相機參數輸出目錄（save_camera_params=True 時必須提供）
    """
    # 驗證必要參數
    if render_mask and mask_save_dir is None:
        raise ValueError("render_mask=True 時必須提供 mask_save_dir")
    if save_camera_params and camera_params_save_dir is None:
        raise ValueError("save_camera_params=True 時必須提供 camera_params_save_dir")
...
if __name__ == '__main__':
    ...
    '''
    # 以下是推論單一影像之作法
    ply_path = 'uninference_tooth/00240433LowerJaw.ply'
    img_save_dir = './'
